...
24. **024 · Base de conocimiento (KB)** — endpoints `/v1/kb/*` en `app/api/routes_kb.py` (categorías CRUD, artículos listado/crear/detalle/update/publicar/archivar/restaurar/versiones) por tenant con permisos RBAC (`kb:read`/`kb:edit`/`kb:publish`), tags relacionales (`kb_article_tags`) y versionado por snapshot. Validado contra FastAPI local (categorías y artículos con `supervisor`). Cierra el gap de la feature 007 del frontend y el plan `features/019-base-conocimiento/`. _(Pendiente futuro: RAG avanzado con filtro por idioma/vigencia — ver Backlog.)_
